...
from metrics_lie.profiles import BALANCED, PERFORMANCE_FIRST, RISK_AVERSE, get_profile
...
def test_null_subgroup_gap_ignored():
    ...
    assert result.meta["per_scenario"]["label_noise"]["subgroup_gap_delta"] == pytest.approx(0.02)
...
def test_meta_includes_per_scenario_data():
    ...
    assert result.meta["per_scenario"]["label_noise"]["metric_mean_delta"] == pytest.approx(-0.02)
...
def test_scenario_scope_echo_subset():
    ...
    assert set(result.scenario_scope["requested_subset"]) == {"label_noise", "class_imbalance"}
