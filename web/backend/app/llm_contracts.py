...
class CompareExplainRequest(BaseModel):
    ...
    intent: str = Field(..., description="Analyst intent (overview, worse, improved, worst_case, new_flags, scenario_focus, component_focus, flag_focus)")
    focus: Optional[FocusContext] = Field(None, description="Optional focus context")
    context: dict = Field(..., description="Comparison context bundle (JSON-serializable)")
...
class CompareExplainResponse(BaseModel):
    ...
    evidence_keys: list[str] = Field(default_factory=list, description="List of evidence keys referenced (scenario:<id>, component:<name>, flag:<code>)")
