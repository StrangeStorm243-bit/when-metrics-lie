...
class DecisionScorecard(BaseModel):
    ...
    contributions: dict[str, float] = Field(..., description="Per-component contributions (weight * value)")
    used_components: list[str] = Field(..., description="List of components that were scored")
...
def build_scorecard(decision_components: DecisionComponents, profile: DecisionProfile) -> DecisionScorecard:
    """
    ...
    meta: dict[str, Any] = {
        ...
        "component_values": {
            k: v for k, v in decision_components.components.items()
            if v is not None
        },
