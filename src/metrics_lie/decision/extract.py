...
def _aggregate_percentile(values: list[float], percentile: float, use_upper: bool = False) -> float:
    """
...
def extract_components(compare_report: dict[str, Any], profile: DecisionProfile) -> DecisionComponents:
    """
    ...
    for comp_name, values in component_lists.items():
        ...
        if mode == "mean":
            ...
            aggregated[comp_name] = _aggregate_percentile(values, percentile, use_upper=use_upper)
        else:
